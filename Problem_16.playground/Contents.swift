...
// Flip and can move to index or index + 1
...
func rowSum(_ triangle: Array<Array<Int>>) -> Array<Array<Int>> {
    ...
    for (index, leftItem) in newTriangle[0].enumerated() {
        if index < newTriangle[0].count - 1 {
            let rightItem = newTriangle[0][index + 1]
            let childItem = newTriangle[1][index]
            let leftSum = leftItem + childItem
            let rightSum = rightItem + childItem
            if leftSum > rightSum {
                newTriangle[1][index] = leftSum
            } else {
                newTriangle[1][index] = rightSum
            }
        }
    }
    newTriangle.remove(at: 0)
    if newTriangle.count != 1 {
        return rowSum(newTriangle)
    }
    return newTriangle
}

maxSumPath(triangle)[0][0]
